...
const StatCard = ({ title, value, percentage, change, icon: Icon, iconBgColor }: { title: string, value: string, percentage?: string, change?: 'up' | 'down', icon: React.ElementType, iconBgColor: string }) => (
    <div className="bg-white dark:bg-slate-80 p-4 rounded-lg shadow flex items-start justify-between">
        <div>
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        if (!hasFetchedRef.current && !state.loading) {
            fetchData();
            hasFetchedRef.current = true;
        }
    }, [fetchTasks, addToast, state.loading]);
    ...
    if (state.error) return <ErrorBoundary><div className="text-red-600">Erreur: {state.error}</div></ErrorBoundary>;
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="w-full mt-1 p-2 border dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-70 dark:text-slate-200" />
                        </div>
                        <div>
                            <label className="text-xs text-slate-600 dark:text-slate-400">Date de fin</label>
                            <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="w-full mt-1 p-2 border dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-70 dark:text-slate-200" />
                        </div>
                        <div>
                            <label className="text-xs text-slate-600 dark:text-slate-400">Département</label>
                            <select name="department" value={filters.department} onChange={handleFilterChange} className="w-full mt-1 p-2 border dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-70 dark:text-slate-200">
                        ...
                        <div>
                            ...
                            <select name="priority" value={filters.priority} onChange={handleFilterChange} className="w-full mt-1 p-2 border dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-70 dark:text-slate-200">
