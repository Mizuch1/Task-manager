...
const CreateTask = () => {
    const { state, fetchUsers, createTask } = useTask();
    ...
    const validationRules = {
        title: { required: true, minLength: 3, message: 'Le titre est requis et doit contenir au moins 3 caractères' },
        description: { required: true, minLength: 10, message: 'La description est requise et doit contenir au moins 10 caractères' },
        dueDate: { required: true, message: 'La date d\'échéance est requise' },
    };

    const { validateForm, getFieldError, errors } = useFormValidation(validationRules);

    useEffect(() => {
        fetchUsers();
        if (state.users.length > 0 && taskData.assignedTo.length === 0) {
            setTaskData(prev => ({ ...prev, assignedTo: [state.users[0].id] }));
        }
    }, [fetchUsers, state.users, taskData.assignedTo.length]);
    ...
    const handleSubmit = async () => {
        if (!currentUser) {
            addToast("Vous devez être connecté pour créer une tâche.", 'error');
            return;
        }
        setSubmitting(true);
        try {
            ...
            addToast("Une erreur est survenue lors de la création de la tâche.", 'error');
            setSubmitting(false);
        }
    };
    ...
    const getAssignedUserName = () => {
        ...
        const user = state.users.find(u => u.id === taskData.assignedTo[0]);
                            ...
                            <div className="p-4 border dark:border-slate-700 rounded-lg">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                     <div><label className="text-sm dark:text-slate-300">Assignés *</label><select name="assignedTo" value={taskData.assignedTo[0]} onChange={handleSelectChange} className="w-full mt-1 p-2 border dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 dark:text-slate-200">
                                        {state.users.map(user => <option key={user.id} value={user.id}>{user.firstName} {user.lastName}</option>)}
                                     </select></div>
