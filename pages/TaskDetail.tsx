...
const TaskDetail = () => {
    ...
    const { state, fetchUsers, updateTask, addComment } = useTask();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setLoading(true);
            await fetchUsers();
            const taskData = state.tasks.find(t => t.id === id) || null;
            setTask(taskData);
            setEditableTask(taskData);
            setLoading(false);
        };
        
        if (state.tasks.length > 0 && !task) {
            fetchData();
        }
    }, [id, state.tasks, fetchUsers, state.tasks.length, task]);
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const updatedTask = await updateTask(editableTask.id, editableTask);
            if (updatedTask) {
                setTask(updatedTask);
    ...
    const handleCommentSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const updatedTask = await addComment(task.id, currentUser.id, newComment);
            if (updatedTask) {
                setTask(updatedTask);
                setEditableTask(updatedTask); 
                addToast('Commentaire ajouté avec succès', 'success');
            }
            setNewComment('');
        } catch (error) {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>, type: 'before' | 'after') => {
        ...
        reader.onload = async () => {
            ...
            try {
                const updatedTask = await updateTask(editableTask.id, updatedFields);
                if (updatedTask) {
                    setTask(updatedTask);
                    setEditableTask(updatedTask);
                    addToast(`Photo ${type} mise à jour avec succès`, 'success');
                }
            } catch (error) {
                setTask(task);
                setEditableTask(task);
                addToast("Échec du téléchargement de l'image", 'error');
            }
        };
